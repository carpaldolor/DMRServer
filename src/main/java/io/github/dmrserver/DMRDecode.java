...
public class DMRDecode{
	...
	public String decode() {
		...
		if( tag.equals("DMRD")) {
			...
			ret = "DMRD seq:"+ti_pad(bar[4])+" src:"+ti(bar,5,3)+" dst:"+ti(bar,8,3)+" rpt:"+ti(bar,11,4) +" slot:"+slot+ " strm:"+ti(bar,16,4) +" data:"+(len-20)+ " frame:"+frameName[frame] +" "+st;
		} 
	...
	public static String ti_pad(byte b) {
		...
		if( ret.length() < 3 ) ret = new String("000").substring(0,3-ret.length())+ret ;
		return ret ; 
	}
